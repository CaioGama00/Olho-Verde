...
\begin{itemize}
    ...
    \item \textbf{JWT (JSON Web Tokens):} Mecanismo utilizado para autenticação e autorização segura.
\end{itemize}
...
O backend está organizado em camadas para separar responsabilidades, facilitando testes e manutenção:

\begin{enumerate}
    \item \textbf{Routes (Rotas):} Definem os \textit{endpoints} da API e direcionam as requisições para os controladores.
    \item \textbf{Controllers (Controladores):} Recebem a requisição, validam parâmetros e chamam os serviços.
    \item \textbf{Services (Serviços):} Contêm a lógica de negócio principal.
    \item \textbf{Data Access (Supabase):} Interação direta com o banco de dados.
\end{enumerate}
...
O sistema utiliza tokens JWT para autenticação. O diagrama de sequência abaixo ilustra o processo de login e uso de token para acessar recursos protegidos.
